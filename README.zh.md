...
EaseMC 是一个开源的 Minecraft 离线启动器，为「只想立刻玩上游戏」的玩家而生：无需手动安装 Java、无需研究版本清单、无需注册账号——打开启动器，它会自动完成游戏下载、运行环境安装，一键进入游戏。
...
- **零配置** — 首次启动自动下载全部游戏文件与 Java 21 运行环境（Azul Zulu），无需手动安装任何东西
...
1. **首次运行** — 启动器检查安装状态，并发下载全部游戏文件（BMCLAPI 镜像优先，失败回退 Mojang 官方源），随后下载并解压 Java 21 运行环境。
